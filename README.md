...
현재(current)/일별(daily)/대기질/특보 예보 섹션과 서버 응답 캐시는 아직 구현되지 않았습니다. 모바일에는 `POST /weather` 계약을 안전하게 소비하는 contract-safe API 클라이언트 경계가 추가되었지만, 아직 화면에는 연결되어 있지 않습니다. 또한 기기에 저장될 지역 한 건의 로컬 모델과 이를 공유 `WeatherRequestV1`으로 변환하는 순수 경계(`apps/mobile/src/locations`)가 추가되었습니다(저장소·권한·화면·실제 호출 미포함).
...
- 모바일 저장 지역 경계(`apps/mobile/src/locations`): 공유 `weatherLocation`을 확장한 저장 지역 strict schema(`kmaGrid`/`isCurrent`/`sortOrder` 로컬 전용 필드)와, 저장 지역을 공유 `WeatherRequestV1`으로 명시적으로 변환하는 순수 함수(저장소·권한·화면·실제 호출 없음)
...
- 모바일 API 클라이언트의 화면 연결, 지역 저장소(collection/store adapter), 위치 권한, 현재 위치 조회
