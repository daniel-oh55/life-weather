...
- 모바일 API 클라이언트의 화면 연결, 저장 지역의 React state/화면 연결과 마이그레이션 실행, 위치 권한, 현재 위치 조회 (지역 저장소의 concrete AsyncStorage provider binding과 provider-neutral hydration manager 자체는 구현됨; 이 둘의 composition과 app-start wiring은 미구현)
