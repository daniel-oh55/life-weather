 ...
 * **injected clock**, the PR #8 scheduled issue-time selector, and caller-supplied grid coordinates
 * into a complete {@link KmaForecastRequest}.
 *
 ...
 * still owns runtime request validation), apply an API-availability delay, or add retry/fallback.
 * The method is named `createScheduledRequest` — not `createAvailableRequest` — precisely because it
 * selects the latest *scheduled* issuance and makes **no** claim that the upstream API data is ready.
 *
 ...
 * the clock **zero** times; each `createScheduledRequest()` call reads it **exactly once**, with no
 * argument, and forwards that epoch value to the selector verbatim (no rounding, truncation, or
 * coercion).
 *
...
export interface KmaForecastRequestFactory {
  /**
   * Build a complete {@link KmaForecastRequest} for the given `input`, dating it to the latest
   * *scheduled* issuance at the injected clock's current instant. Reads the clock **exactly once**
   * and returns a **fresh** request object every call.
   */
...
/**
 * Create a request factory bound to an injected {@link KmaForecastRequestClock}. Pure construction:
 * it does **not** call the clock, call the selector, read the environment, perform I/O, register a
 * listener, or start a timer — the returned object merely closes over `clock`. The same instance is
 * safe to call many times; it holds no mutable state and each call is independent of any previous
 * one.
 */
export function createKmaForecastRequestFactory(
  ...
  return {
    createScheduledRequest(input) {
      ...
      // The selector owns the KST schedule rule; it throws a RangeError verbatim for an invalid
      // epoch or an unsupported product — this factory neither catches nor re-wraps that.
      const { baseDate, baseTime } = selectLatestKmaForecastBaseTime({
