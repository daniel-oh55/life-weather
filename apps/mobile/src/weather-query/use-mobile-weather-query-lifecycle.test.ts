...
// ---------------------------------------------------------------------------
// `react`'s `useEffect` is replaced with a minimal fake so the hook can be called as a plain
// function (there is no real renderer/dispatcher in this Node-based setup). `useEffect` is captured
// rather than auto-run: each test decides for itself when a captured effect (and its returned
// cleanup) actually executes, the same way React would decide based on the dependency array — which
// is itself asserted directly in the "no additional request" test below.
// ---------------------------------------------------------------------------
...
// verifies only *when* the hook calls `request`/`reset`, never the store's own state machine
// (covered by `mobile-weather-query-store.test.ts`).
// ---------------------------------------------------------------------------
...
async function loadHook() {
  const { useMobileWeatherQueryLifecycle } = await import('./use-mobile-weather-query-lifecycle');
  return useMobileWeatherQueryLifecycle;
}
...
// ---------------------------------------------------------------------------
// Hook call alone never requests/resets — only running the captured effect can.
// ---------------------------------------------------------------------------

describe('hook call alone', () => {
  it('requests/resets nothing merely by being called (only the captured effect can)', async () => {
    const useMobileWeatherQueryLifecycle = await loadHook();

    useMobileWeatherQueryLifecycle(ready([record('a', 0)], 'a'));
  ...
  it('requests only after the captured effect actually runs', async () => {
    const useMobileWeatherQueryLifecycle = await loadHook();
...
describe('no request outside a validated READY selection', () => {
  it.each([
    ...
    ['EMPTY', empty],
    ['ERROR', errorSnapshot],
  ] as const)('requests 0 times and resets 0 times for %s', async (_label, build) => {
    const useMobileWeatherQueryLifecycle = await loadHook();

    useMobileWeatherQueryLifecycle(build());
    latestEffect().callback();
  ...
  it('requests 0 times and resets when the selected id is not present in locations', async () => {
    ...
    useMobileWeatherQueryLifecycle(snapshot);
    latestEffect().callback();
...
describe('a validated READY selection', () => {
  ...
  it('does not request again when only a non-selected location changes (same status/selectedId)', async () => {
    ...
    useMobileWeatherQueryLifecycle(first);
    latestEffect().callback();
    expect(mobileWeatherQueryStoreMock.request).toHaveBeenCalledTimes(1);

    useMobileWeatherQueryLifecycle(second);
...
// ---------------------------------------------------------------------------
// Selection change and unmount cleanup.
// ---------------------------------------------------------------------------
...
describe('cleanup', () => {
  it("resets/aborts A's query before requesting B's when the selection changes", async () => {
    ...
    useMobileWeatherQueryLifecycle(readyA);
    ...
    useMobileWeatherQueryLifecycle(readyB);
    latestEffect().callback();
    expect(mobileWeatherQueryStoreMock.request).toHaveBeenNthCalledWith(
      2,
  ...
  it('resets when leaving READY, even mid-flight (via the previous effect\'s own cleanup)', async () => {
    ...
    useMobileWeatherQueryLifecycle(loading());
