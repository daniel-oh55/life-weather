...
describe('normalizeKmaWeatherCondition — PTY codes for ultra-short forecast (초단기예보)', () => {
  // 0(없음), 비(1), 비/눈(2), 눈(3), 소나기(4), 빗방울(5), 빗방울눈날림(6), 눈날림(7).
  // 소나기(4) is shared with 단기예보; 5/6/7 are 초단기예보-only.
  const precipCases: Array<[string, KmaWeatherCondition, string]> = [
    ['1', 'RAIN', '비'],
    ['2', 'SLEET', '비/눈'],
    ['3', 'SNOW', '눈'],
  ...
  it('maps shared PTY code 4 (소나기) to SHOWER under ultra-short forecast', () => {
    // PTY 4 is defined for BOTH products in the 2607 guide (초단기예보 소나기), so it is
    // SHOWER here — not UNKNOWN. Only 5/6/7 are 초단기예보-only.
    expect(
      normalizeKmaWeatherCondition({
        product: ULTRA,
        skyCode: '1',
