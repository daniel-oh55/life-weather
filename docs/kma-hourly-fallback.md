...
- `previousIssuance`는 **fallback-attempted branch에만** 존재하며 `plan.previous`에서 파생합니다. previous
  request가 plan에 있어도 실제로 **전송된** branch에서만 노출하므로, planned-but-unsent previous request의
  identity는 trace에 절대 새어 나가지 않습니다.
...
`previous`·`previousIssuance`·`fallbackReason` key는 없습니다. previous request가 전송되지 않았으므로
`previousIssuance`도 반드시 부재합니다.
...
- previousIssuance는 previous attempt가 실제 실행된 branch에만 존재(planned-but-unsent는 노출 안 함)
