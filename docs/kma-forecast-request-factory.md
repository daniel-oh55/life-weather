...
이 factory는 요청 **조립까지만** 담당합니다. Provider 호출, hourly service 자동 연결, 위경도→격자
변환, API availability delay, retry/fallback, HTTP route는 포함하지 않습니다(후속 PR).
...
- factory → hourly service 연결은 PR #10의 **scheduled facade**
  ...
  **자체의 계약(injected clock 정확히 1회, selector 1회, product/baseDate/baseTime/nx/ny만 반환)은
  변경되지 않았습니다.**
- 다만 실제 production clock(system clock adapter)과 그것으로 factory를 조립하는 **composition
  root는 아직 없습니다**(후속 PR). facade는 injected collaborator만 연결할 뿐 production 인스턴스를
  만들지 않습니다.
...
1. ~~factory와 hourly service를 연결하는 application facade~~ — **PR #10에서 완료**
   ...
   다만 system clock adapter와 production composition root는 여전히 후속 PR입니다.
2. system clock adapter와 production composition root(`createKmaForecastRequestFactory(systemClock)`,
   Provider 생성, facade 인스턴스 조립)
3. 위경도 → KMA grid(nx/ny) 변환
4. API availability fallback/retry 정책
