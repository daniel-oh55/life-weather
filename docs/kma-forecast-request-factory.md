...
## Provider·hourly service 자동 연결 없음

- factory는 Provider를 생성·호출하지 않고, `KmaHourlyForecastService`를 자동으로 잇지 않습니다.
...
- caller/composition 계층이 향후 factory → service 순서로 연결합니다([kma-hourly-service.md](./kma-hourly-service.md)).
...
1. factory와 hourly service를 연결하는 application facade(composition root, system clock adapter)
