...
> 예정입니다.
>
> **PR #18 갱신.** PR #18이 그 candidate 후보를 조립하는 **별도** factory
> (`createKmaFallbackRequestPlanFactory`, [kma-fallback-request-plan.md](./kma-fallback-request-plan.md))를
> `apps/api/src/services`에 추가했습니다. 두 factory의 책임은 구분됩니다: **이** single request factory는
> 한 base time으로 **request 1개**를 만들고, 신규 fallback request-plan factory는 PR #16 candidate pair로
> **request 2개**(`{ primary, previous }`)를 만듭니다. 신규 factory는 **이** factory를 (한 번도, 두 번도)
> **호출하지 않고**, 이 factory의 공개 API·schedule-only default·production wiring도 **변경하지
> 않습니다.** 신규 factory는 여전히 실제 fallback을 실행하지 않으며 production에 연결되지 않았습니다.
...
v6 / PR #18 / 2026-07 (별도 fallback request-plan factory 추가 — 이 factory 불변)
- PR #18이 candidate pair를 request 2개로 조립하는 별도 factory(createKmaFallbackRequestPlanFactory,
  apps/api/src/services)를 추가; 이 single request factory는 계속 request 1개만 생성
- 신규 factory는 이 factory를 두 번(또는 한 번도) 호출하지 않고, 공개 API·schedule-only default·
  production wiring을 변경하지 않음
- 두 factory 책임 구분(1개 vs 2개 request), 신규 factory는 fallback 실행/ production 미연결
```
