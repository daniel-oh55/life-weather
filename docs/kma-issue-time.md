...
따라서 이 함수(`selectLatestKmaForecastBaseTime`)는 다음을 추가하지 않습니다: 임의의 publication
lag(예 10/15/20분), safety margin, 직전 발표시각 fallback, retry, live availability probe. 활용가이드의
`API 제공 시간`(단기 `~02:10`, 초단기 `~HH45`)은 이 자료 제공 지연을 뜻하며 **이 schedule selector의
관심사가 아닙니다.** 이 함수는 계속 schedule-only 책임만 가집니다.

> **PR #14 / PR #15 갱신.** 위 활용가이드 `API 제공 시간`(단기 발표 +10분, 초단기 발표 +15분)을 반영하는
> 일은, 이 schedule selector를 바꾸는 대신 **별도의 순수 availability-delay selector**
> (`selectLatestKmaForecastBaseTimeAfterAvailabilityDelay`, [kma-api-availability-time.md](./kma-api-availability-time.md))로
> PR #14에서 추가했습니다. 그 selector는 `reference − delay`에 이 schedule selector를 재사용할 뿐이며,
> 이 함수(`selectLatestKmaForecastBaseTime`) 자체는 availability-aware로 **바뀌지 않았습니다** — 의미·
> 동작·테스트 기대값 모두 그대로입니다. **PR #15**에서 request factory에 selector 주입 seam이 생겨
> production composition이 그 availability-delay selector를 주입하지만, request factory의 **default는
...
- **selector를 소비하는 계층(PR #9, 구현 완료).** 이 pure selector는 PR #8에서 완료됐고, PR #9의
  ...
  [kma-scheduled-hourly-facade.md](./kma-scheduled-hourly-facade.md))가 그 factory와 hourly service를
  이어 **selector → request factory → scheduled facade → hourly service** 흐름을 완성합니다. selector
  자신의 책임과 발표 schedule 근거는 변경되지 않았고, 이 selector·factory·facade 어느 것도 API
  availability를 보장하지 않습니다(발표 schedule만 선택). **PR #11**에서는 production composition
  root([kma-production-composition.md](./kma-production-composition.md))가 **system clock → request
  factory → selector** 흐름을 실제 dependency로 조립합니다 — selector는 여전히 순수하고 시스템
  clock을 직접 읽지 않으며(clock은 request factory에 주입), availability 보장도 그대로 없습니다.
  **PR #15**에서는 그 production composition이 request factory의 selector seam에 PR #14
  availability-delay selector를 주입해 production 흐름이 `availability-delay selector`를 사용하도록
  바뀌었지만, request factory의 **default selector는 여전히 이 schedule selector**이므로 selector를
  생략한 direct caller의 schedule-only 결과는 그대로입니다([kma-api-availability-time.md](./kma-api-availability-time.md)).
- 남은 후속:
  1. ~~factory → hourly service를 잇는 application facade~~ — **PR #10에서 완료**.
     ~~system clock adapter와 production composition root~~ — **PR #11에서 완료**
     ([kma-production-composition.md](./kma-production-composition.md)).
  2. ~~위경도 → KMA grid(nx/ny) 변환~~ — **PR #12에서 완료**
     ([kma-grid-conversion.md](./kma-grid-conversion.md)), **PR #13에서 소비**
     ([kma-location-scheduled-hourly.md](./kma-location-scheduled-hourly.md)).
  3. ~~공식 API 제공 지연(단기 +10분, 초단기 +15분)을 반영하는 순수 selector~~ — **PR #14에서 별도
     함수로 완료**(`selectLatestKmaForecastBaseTimeAfterAvailabilityDelay`,
     [kma-api-availability-time.md](./kma-api-availability-time.md)). 이 schedule selector는 그대로입니다.
  4. ~~availability-delay selector를 소비하는 명시적 request factory/wiring~~ — **PR #15에서 완료**:
     request factory에 selector 주입 seam을 추가하고 production composition이 PR #14 selector를 주입
     ([kma-forecast-request-factory.md](./kma-forecast-request-factory.md),
     [kma-production-composition.md](./kma-production-composition.md)). direct factory default는 여전히 이
     schedule selector입니다.
  5. live availability fallback/retry 정책
...
v2 / PR #11 / 2026-07 (production composition에서 조립)
- system clock → request factory → selector 흐름이 production composition에서 실제 dependency로 조립됨
- selector는 계속 순수하고 시스템 clock을 직접 읽지 않음(공식 일정·hash 설명 변경 없음)
- availability 보장 없음, route 미연결

v3 / PR #14 / 2026-07 (별도 availability-delay selector 추가, 이 selector는 불변)
- 활용가이드 `API 제공 시간`(단기 +10분·초단기 +15분)을 반영하는 순수 selector를 PR #14에서 별도 함수
  (selectLatestKmaForecastBaseTimeAfterAvailabilityDelay)로 추가 — 이 schedule selector의 의미·동작·
  테스트 기대값은 변경 없음(availability-aware로 바뀌지 않음)
- request factory는 여전히 이 schedule selector(createScheduledRequest)를 사용, 신규 selector 미주입
- live fallback/retry는 여전히 후속

v4 / PR #15 / 2026-07 (production wiring — 이 selector는 여전히 불변, default 역할 유지)
- PR #15가 request factory selector seam으로 PR #14 availability-delay selector를 production composition에
  주입했으나, 이 schedule selector의 의미·동작·테스트 기대값은 변경 없음
