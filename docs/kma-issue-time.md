...
> **PR #14 갱신.** 위 활용가이드 `API 제공 시간`(단기 발표 +10분, 초단기 발표 +15분)을 반영하는 일은,
> 이 schedule selector를 바꾸는 대신 **별도의 순수 availability-delay selector**
...
> 동작·테스트 기대값 모두 그대로입니다. PR #9의 request factory(`createScheduledRequest`)도 여전히 이
> schedule selector를 사용하며, PR #14 신규 selector는 아직 어느 request factory/composition에도
> 주입되지 않았습니다. live availability fallback/retry는 여전히 후속입니다.
...
- 남은 후속:
  ...
  3. ~~공식 API 제공 지연(단기 +10분, 초단기 +15분)을 반영하는 순수 selector~~ — **PR #14에서 별도
     ...
     [kma-api-availability-time.md](./kma-api-availability-time.md)). 이 schedule selector는 그대로이며,
     신규 selector는 아직 어느 request factory/composition에도 주입되지 않았습니다.
  4. availability-delay selector를 소비하는 명시적 request factory/wiring, live availability
     fallback/retry 정책
  5. `WeatherOverview`/`SourceMetadata` 조립
  6. `/weather` API route
