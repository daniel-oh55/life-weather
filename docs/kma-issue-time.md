...
- **selector를 소비하는 계층(PR #9, 구현 완료).** 이 pure selector는 PR #8에서 완료됐고, PR #9의
  ...
  availability를 보장하지 않습니다(발표 schedule만 선택). 실제 clock/Provider를 조립하는 composition
  root는 아직 없습니다.
- 남은 후속:
  1. ~~factory → hourly service를 잇는 application facade~~ — **PR #10에서 완료**. system clock
     adapter와 production composition root는 여전히 후속.
