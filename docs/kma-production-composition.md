...
- **route 연결** — composition root는 아직 어떤 route에도 연결되지 않았습니다. `/weather`는 없고,
  `/health`와도 무관합니다.
- **위경도 → KMA grid(nx/ny) 변환** — 순수 converter(`convertKmaLatitudeLongitudeToGrid`)는 PR #12에서
  `weather-core`에 존재하지만, 이 production composition은 아직 그 converter를 **조립하지 않습니다.**
  반환된 facade는 여전히 caller가 이미 계산한 nx/ny를 요구하며, startup/route에도 연결되지 않았습니다.
...
2. PR #12 converter(`convertKmaLatitudeLongitudeToGrid`)를 소비하는 latitude/longitude location
   adapter와 이를 이 production composition에 연결하는 wiring.
