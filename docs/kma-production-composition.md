...
- **PR #19 fallback orchestration service** — PR #16 candidate selector · PR #17 classifier · PR #18
  ...
  추가됐지만, **이 production composition은 그것도 아직 소비하지 않습니다.** 현재 두 production facade
  (scheduled·location)는 그대로 각각 facade 호출당 **Provider 최대 1회**를 유지합니다. 별도 production
  fallback composition(primary 후 eligible 시 previous 최대 1회)은 **다음 PR(PR #20)** 예정이며,
  그때까지 retry/fallback 실행·live readiness 보장은 없습니다.
