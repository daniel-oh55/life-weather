...
- **미연결**: 이 root는 `apps/api/src/index.ts`·startup·`/weather` route에 연결되지 않았고, location(위·
  경도) fallback root는 후속 PR입니다.

## 후속 범위

1. location(위·경도) fallback facade/composition(grid fallback root 앞단에 PR #12 converter 연결).
2. 이 composition root들을 소비하는 `/weather` route 계약과 query validation.
3. HTTP status mapping.
4. final primary/previous result selection.
