...
분류), **아직 production graph에서 조합되지 않았습니다** — 기존 Provider·service·facade·composition의
runtime과 공개 API는 변경하지 않았고(production 동작 불변, facade 호출당 KMA request 최대 1회 유지),
실제 fallback 실행·retry·route·cache는 여전히 미구현입니다.
...
1회 후 eligible이면 previous 최대 1회를 실행하지만, 이 orchestration은 **아직 production graph(scheduled/
location composition·route)에 연결되지 않았습니다** — 기존 Provider·service·facade·composition의 runtime과
