...
반환하더라도, 실제 PR #23 assembler의 nonempty guard가 integrated service에서도 그대로 동작해
**synchronous `ZodError`**로 reject됩니다. 이때 partial success result를 반환하지 않고, selected source
metadata를 overview로 노출하지 않으며, input을 mutate하지 않습니다.
