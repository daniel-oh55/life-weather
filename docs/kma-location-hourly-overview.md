...
## composition / route / cache

- 이 service의 로직 자체는 어떤 production composition을 **내장하지 않습니다**(순수 orchestration seam,
  collaborator만 주입).
- **PR #27 갱신**: PR #27의 다섯 번째 callable root
  (`createKmaLocationHourlyOverviewCompositionFromEnv`,
  [kma-location-hourly-overview-composition.md](./kma-location-hourly-overview-composition.md))가 이
  service를 PR #21 location fallback facade·PR #26 live resolver와 조립합니다. 이로써 callable production
  composition root 수는 **5개**(grid/location scheduled·grid/location fallback + location hourly
  overview)가 됩니다. 이 service의 factory signature·runtime·public types는 PR #27로도 **불변**입니다.
- `/weather` route·startup·HTTP status/envelope mapping·cache/stale-data는 PR #27로도 여전히
  **미구현**입니다(다섯 번째 root조차 아직 startup/route에 연결되지 않음).
...
3. ~~PR #24 service + PR #26 resolver를 production composition에 조립 (PR #27)~~ — PR #27
   `createKmaLocationHourlyOverviewCompositionFromEnv`
...
5. `/weather` route와 internal application result를 `overview`만 매핑하는 mobile-facing serialization
   mapper.
6. cache/stale-data.
7. authenticated KMA E2E.
...
v4 / PR #27 / 2026-07 (production composition 구현; 이 service runtime은 불변)
- createKmaLocationHourlyOverviewCompositionFromEnv(다섯 번째 callable root)가 이 service를 PR #21 facade·
  PR #26 resolver와 조립 ([kma-location-hourly-overview-composition.md](./kma-location-hourly-overview-composition.md))
- callable production composition root 수 4 → 5; 이 service의 factory signature·runtime·public types는 불변
- /weather route·startup wiring·overview-only mobile serialization mapper·cache는 여전히 후속
```
