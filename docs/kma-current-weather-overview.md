...
이 문서는 PR #72에서 추가한 **pure, synchronous 함수** 한 개
(`assembleKmaCurrentWeatherOverview`)의 책임과 경계를 기록합니다. 이 함수는
...
- 입력은 `location`과 `current` 두 field만 갖는 `KmaCurrentWeatherOverviewInput`입니다 — 새 result
  union, stage discriminator, execution trace를 요구하지 않습니다(hourly assembler의
...
- `apps/api/src/services/index.ts`에서 `assembleKmaCurrentWeatherOverview`와
  `KmaCurrentWeatherOverviewInput`을 export합니다.
...
## `sources: []` — provenance는 후속 PR 책임

contracts `weatherOverview` schema는 `current === null ↔ 'CURRENT' ∈ missingSections`만 강제하고,
`current`가 존재할 때 `SourceMetadata`가 있어야 한다는 규칙은 **강제하지 않습니다.** 이 assembler는
그 여지를 이용해 provenance를 **발명하지 않습니다** — 다음을 결정하거나 생성하지 않습니다.
...
`CurrentWeather.observedAt`과 (아직 존재하지 않는) current `SourceMetadata.observedAt`의 관계도 이
PR에서 새 규칙으로 정의하지 않습니다. current `SourceMetadata` 조립은 **후속 PR의 책임**입니다.
...
- `input`/`input.location`/`input.current`를 mutate하지 않습니다 — deep-frozen input에서도 정상
  동작합니다.
...
연결되지 않았습니다. 구체적으로 이 PR 이후에도 다음은 여전히 미구현입니다.

- current `SourceMetadata` resolver / 정책
...
즉 이 PR 이후에도 `POST /weather`의 production 응답에서 `current`는 계속 missing입니다.
