...
- **지원 위치 (no fallback)**: fallback service의 `{ fallbackAttempted: false, primaryIssuance, primary }`를
  그대로 반환합니다.
- **지원 위치 (fallback)**: fallback service의 `{ fallbackAttempted: true, fallbackReason, primaryIssuance,
  primary, previousIssuance, previous }`를 그대로 반환합니다. `fallbackAttempted: true`는 previous
  hourly-service 호출이 **일어났다**는 뜻이지 previous가 데이터를 담았다는 뜻이 아닙니다. PR #25부터 execution
  trace는 각 attempt의 sanitized issuance identity(`product`/`baseDate`/`baseTime`)를 함께 담으며,
  `previousIssuance`는 previous가 실제 실행된 fallback branch에만 존재합니다 — location facade는 이 trace를
  그대로 통과시킬 뿐 issuance를 만들거나 확장하지 않습니다.
- location facade는 어느 branch에도 `locationResolved`·`grid`·`latitude`·`longitude`·`selected`·`final`·
  `fallbackUsed`·`source`·`stale`·`provider`·`request`·`plan` 같은 field를 **추가하지 않습니다**(sanitized
  issuance identity는 fallback service가 이미 trace에 넣은 것으로, facade가 추가하는 field가 아닙니다).
...
- **primary/previous 최종 선택 / result merge / `fallbackUsed` API field** — 이 facade에는 **없음**.
  facade는 계속 PR #19 execution trace(또는 `LOCATION` branch)만 반환합니다. PR #22에서 primary/
  ...
  [kma-hourly-weather-overview.md](./kma-hourly-weather-overview.md))가 추가됐고, PR #24 application
  service(`createKmaLocationHourlyOverviewService`)가 이 orchestration을 facade **바깥에서** 이미
  구현합니다 — `LOCATION` branch를 먼저 narrow해 지원되지 않으면 facade 결과를 그대로 반환하고, 지원되는
  `KmaHourlyFallbackServiceResult` trace만 selector로 변환한 뒤 그 selection을 PR #23 assembler에
  전달합니다. selector·resolver·assembler는 어느 것도 facade에 **내장되지 않으며**, facade가 반환하는
  `LOCATION` branch를 직접 처리하지도 않습니다. facade의 공개 API·Promise identity·unsupported/invalid
  location 처리는 **불변**입니다(facade는 selection도 assembly도 수행하지 않음). 자세한 pipeline은 아래
  `facade consumer (PR #24)` section을 참조하세요.
...
- selector는 facade에 내장되지 않음(PR #22 당시 후속 consumer가 LOCATION narrow 후 소비하도록 계획; 이후 PR #24 application service로 구현 완료)
...
- assembler도 selector도 facade에 내장되지 않음(PR #23 당시 후속 application service가 LOCATION narrow →
  selector → assembler 순으로 호출하도록 계획; 이후 PR #24에서 구현 완료)
- location facade 공개 API·Promise identity·unsupported/invalid location 처리 불변
