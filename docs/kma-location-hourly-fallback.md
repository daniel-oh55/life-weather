...
primary/previous selection policy(PR #22 selector)·hourly-only `WeatherOverview`/`SourceMetadata`
assembler(PR #23)·location fallback facade/composition(PR #21)·PR #20 grid fallback wiring은 모두 구현
완료 상태입니다. 남은 책임은 이 순수 component들을 `LOCATION` branch narrow와 함께 엮는 application service와
그 wiring이며, 다음 순서로 정리됩니다.

1. `KmaLocationHourlyFallbackResult`에서 `LOCATION` branch를 먼저 narrow해 처리하는 application service.
...
3. selected source의 provenance context를 결정하고, PR #23 assembler(`assembleKmaHourlyWeatherOverview`)를
   호출해 selection과 hourly-only `WeatherOverview`를 함께 반환.
4. 이 application service의 production composition wiring.
