...
- **primary/previous 최종 선택 / result merge / `fallbackUsed` API field** — 이 facade에는 **없음**.
  ...
  [kma-hourly-fallback-selection.md](./kma-hourly-fallback-selection.md))가 추가됐지만, 이 selector는
  **facade에 아직 내장되지 않았습니다** — selector는 `LOCATION` branch가 아니라 **성공적으로 grid로
  변환된 뒤 반환된 `KmaHourlyFallbackServiceResult` trace**에만 적용됩니다. 후속 assembler가 먼저
  `LOCATION` branch를 narrow한 뒤 selector를 호출하는 것이 책임 분담이며, facade의 공개 API·Promise
  identity·unsupported/invalid location 처리는 **불변**입니다(facade는 어떤 selection도 수행하지 않음).
...
primary/previous selection policy(PR #22 selector)·location fallback facade/composition(PR #21)·PR #20 grid
fallback wiring은 모두 구현 완료 상태입니다. 남은 책임은 selector를 소비하는 assembler와 그 wiring이며, 다음
순서로 정리됩니다.

1. `KmaLocationHourlyFallbackResult`에서 `LOCATION` branch를 먼저 narrow해 처리하는 assembler.
2. 성공적으로 grid로 변환된 뒤 반환된 successful `KmaHourlyFallbackServiceResult` trace에 PR #22
   selector(`selectKmaHourlyFallbackResult`)를 적용.
3. selected hourly와 execution trace를 `WeatherOverview`/`SourceMetadata`로 조립.
4. assembler의 production composition wiring.
5. `/weather` route와 HTTP status mapping.
