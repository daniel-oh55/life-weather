...
| 확인 날짜 | 2026-07-16 |
| 확인 주체 | Claude (Claude Code) — 공식 자료 직접 확인 |
...
### 이번 PR에서 실제로 확인에 사용한 공식 자료

JSON envelope와 각 필드의 실제 scalar 타입은 아래 **공식** 자료로 직접 교차 확인했습니다.
...
### 확인된 실제 JSON scalar 타입

공식 JSON에서 확인된 타입만 허용하며, `z.coerce`를 쓰지 않고 문자열↔숫자 자동 변환을 하지
않습니다. 단기예보와 초단기예보 item의 실제 타입은 **동일**합니다.

| 필드 | 위치 | JSON 타입 | 스키마 |
| --- | --- | --- | --- |
| `resultCode` | header | string (예 `"00"`) | `z.string()` |
| `resultMsg` | header | string | `z.string()` |
| `dataType` | body | string (예 `"JSON"`) | `z.string()` |
| `pageNo` | body | number | 정수 `>= 1` |
| `numOfRows` | body | number | 정수 `>= 1` |
| `totalCount` | body | number | 정수 `>= 0` |
| `baseDate` | item | string `YYYYMMDD` | 실제 달력 날짜 검증 |
| `baseTime` | item | string `HHmm` (`HH24MI`) | 시 00–23 / 분 00–59 |
| `category` | item | string | non-empty, 앞뒤 공백 없음 |
| `fcstDate` | item | string `YYYYMMDD` | 실제 달력 날짜 검증 |
| `fcstTime` | item | string `HHmm` (`HH24MI`) | 시 00–23 / 분 00–59 |
| `fcstValue` | item | string 또는 `null` | `z.string().nullable()` |
| `nx` | item | number | 정수 `>= 0` |
| `ny` | item | number | 정수 `>= 0` |
...
| `resultCode` | 응답 명세 항목크기 `2`, 샘플 `"00"`. XML 예시 본문은 `0`으로 축약 표기 | string `"00"` (숫자 coercion 금지) | 명세 샘플·에러코드 표(`00` NORMAL_SERVICE) 우선. XML 축약은 근거로 쓰지 않음 |
| `nx`/`ny` 범위 | 항목크기 `2`로 표기되나 샘플이 `127`(3자리) | 정수 `>= 0`, 상한 미적용 | 문서가 유효 범위를 신뢰성 있게 명시하지 않아 상한을 추측하지 않음. 위경도→grid 변환은 이 PR 범위 아님 |
| `fcstValue` 타입 | 항상 문자열(TMP 등 "실수로 제공" 자료도 `"-2"` 형태). 공식 JSON에 number·null 표기는 확인 안 됨 | `string \| null` (number 불가) | 아래 "fcstValue 정책" 참조. **남은 위험**: 공식 샘플에 명시적 `null`은 없음(방어적 허용) |
...
1. `response.header`(문자열 `resultCode`/`resultMsg`)가 유효하지 않으면 → `INVALID_RESPONSE`.
2. header는 유효하지만 `resultCode`가 성공 코드가 아니면 → `UPSTREAM_ERROR`.
...
type KmaForecastResponseError =
  | { kind: 'UPSTREAM_ERROR'; resultCode: string; resultMessage: string }
...
- 임의의 KMA 오류 코드를 enum으로 제한하지 않고 원본 `resultCode`·`resultMsg`를 보존합니다.
- 서비스 키·요청 URL·raw response 전체를 error에 포함하지 않습니다.

공식 에러코드 표(가이드 원문, `resultCode`는 문자열):
...
- 잘못된 날짜/시간, 잘못된 scalar 타입, 비정상 pagination, 오염된 category
...
- issue 순서는 `(path, message)` 기준 코드유닛 비교로 정렬해 **결정론적**입니다.
...
어떤 값도 오류로 새어나갈 수 없습니다. 그래서 upstream error는 공식 header 2개 문자열만,
invalid response는 `path`+`message`만 보존합니다.
...
- 공식 JSON에서 `fcstValue`는 **항상 문자열**입니다(TMP 등 "실수로 제공" 자료도 `"-2"`,
  `"6.2"`처럼 문자열 인코딩). 따라서 **number는 허용하지 않고**(coercion 금지), object/array도
  거부합니다. `KmaForecastScalar = string`으로 정의합니다.
...
- **남은 위험 / 근거의 성격.** 공식 샘플에서 `fcstValue`가 실제로 `null`로 오는 사례는 확인되지
  않았습니다. 그럼에도 명시적 `null`을 **방어적으로 허용**하는 이유는, field-presence 모델이
  "필드 존재+null"과 "필드 미존재"를 구분해야 하기 때문입니다(PR #3
  [kma-normalization.md](./kma-normalization.md)가 요구한 항목). `null`을 값으로 강제 변환하지
  않으며, 실제로 `null`이 오지 않는다면 `NULL` 상태가 파싱 결과에서 도달 불가능해질 뿐 모델은
...
`category`는 strict enum이 아니라 non-empty 코드 문자열로만 검증하므로, KMA가 새 category를
추가해도 raw boundary는 이를 **보존**합니다. 공통 상태 변환(SKY/PTY/PCP/SNO 등)은
...
resultCode "03"(NODATA) 등 비성공 코드           → UPSTREAM_ERROR (resultCode/resultMsg 보존)
성공 코드 + item 빈 배열                          → ok:true, slots 없음
category item 미존재                              → getKmaForecastField → ABSENT
item 존재 + fcstValue null                       → NULL
item 존재 + 값                                   → VALUE
```

## 실제 HTTP Provider는 아직 없음

이 PR은 원본 JSON 구조 검증·분류·slot 추출까지만 구현합니다. `fetch`, `KMA_SERVICE_KEY`,
query/URL 인코딩, timeout/`AbortSignal`, retry, HTTP status·content-type 처리, Provider
class, weather-core normalizer 연결, API route는 **구현되어 있지 않으며** PR #5에서 다룹니다.
`.env.example`의 `KMA_SERVICE_KEY` 변수명은 이 PR에서 읽지 않습니다.
