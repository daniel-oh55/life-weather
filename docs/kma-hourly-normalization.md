...
- **공식 근거:** 초단기예보(`getUltraSrtFcst`)도 POP(강수확률)를 제공합니다. 아래 "공식 자료별 POP
  표기"에서 보듯 현재 **두 공식 자료가 모두 초단기예보에 POP를 포함**하며, 제공 시작 시각 표기만
  다릅니다(웹 변수 목록 `12 KST` / 활용가이드 DOCX `11 KST`).
- 따라서 최신 발표분의 초단기예보 slot에는 POP `VALUE`가 정상적으로 존재할 수 있습니다. 값이
  존재하면 다른 상품과 동일하게 `parseKmaPercentage`로 `0~100` percentage로 파싱합니다.
- POP가 `ABSENT`(제공 시작 이전 발표분·부분 응답·방어적 누락 등)이거나 `NULL`이면 오류가 아니라
...
- normalizer는 **발표일자·발표시각 조건을 하드코딩하지 않고** slot의 field presence만 해석합니다.
  단기·초단기 어느 상품이든 POP 조립 코드는 동일합니다(`getKmaForecastField` → presence 분기 →
  존재하면 `parseKmaPercentage`, 아니면 `null`). 그러므로 아래 공식 자료의 시작 시각 표기 차이는
  runtime 동작에 영향을 주지 않습니다(날짜·시각 분기 없음, presence만 처리).
- **live 검증 대상:** 실제 인증된 공공데이터포털 JSON에서 초단기예보 POP가 반환되는 구체적
  형태(키 존재·값 표기·실제 제공 시점)는 실제 키를 이용한 후속 live 통합 검증으로 확인할 항목입니다.

### 공식 자료별 POP 표기 (제공 시작 시각 차이)

현재 두 공식 자료는 **모두 초단기예보에 POP를 포함**합니다. 차이는 "POP 누락 여부"가 아니라
**제공 시작 시각 표기(`11 KST` vs `12 KST`)** 이며, 어느 자료가 잘못됐는지 추정하지 않고 정확한
불일치 원인도 확인되지 않았음을 기록합니다.

| 공식 자료 | POP 포함 | 제공 시작 시각 표기 | 확인 주체 |
| --- | --- | --- | --- |
| API 허브 웹 변수 목록 (`apiList.do?seqApi=10`, `2.2 초단기예보` 예보변수) | 포함 | `POP(강수확률, 2026.6.23. 12KST 이후부터 제공)` | Claude (Claude Code) — 이번 작업에서 직접 확인 (2026-07-17) |
| API 허브 활용가이드 DOCX (`단기예보조회서비스_API활용가이드_260623.docx`) | 포함 (초단기예보 코드값 표에 `POP 강수확률 %`) | 주석 `※ 초단기예보 강수형태(POP)요소 확대 제공: 2026.6.23. 11KST 이후` | Claude (Claude Code) — 이번 작업에서 재다운로드·SHA-256 재검증 (2026-07-17), Codex 최종 리뷰 결과와 일치 |

- DOCX(`_260623.docx`) SHA-256(재검증): `99504eafccd7fef184f3e26a25a71de4804461826528ef2223ff72027e500d30`
  (428,062 bytes). 이 값은 **`_260623.docx` 배포본 고유 값**이며, 아래 공공데이터포털 2607 ZIP(및
  그 내부 DOCX)의 SHA-256과 **혼동하지 않습니다**.

| 자료(배포본) | SHA-256 | 검증 주체 |
| --- | --- | --- |
| ① API 허브 웹 변수 목록 | (웹 페이지 — 파일 해시 없음) | Claude (Claude Code), 2026-07-17 직접 확인 |
| ② API 허브 `단기예보조회서비스_API활용가이드_260623.docx` | `99504eafccd7fef184f3e26a25a71de4804461826528ef2223ff72027e500d30` | Claude (Claude Code) 2026-07-17 재검증, Codex 최종 리뷰 일치 |
| ③ 공공데이터포털 활용가이드 ZIP (`…_2607.zip`) | `07f53cd9d6d6512bce6ef870d54cb740046a0a949896e6855caecf739fb8842e` | Codex 독립 리뷰(PR #3) |
| ③-내부 위 ZIP 내부 DOCX (2607) | `20d855aa3071a2bdda6dce3c13bab6428ebb02f8d4a30688e26ed0851d6d0848` | Codex 독립 리뷰(PR #3) |

> 정리: 웹 변수 목록과 현재 DOCX(`_260623.docx`)는 **모두** 초단기예보 POP를 포함하며, 제공 시작
> 시각 표기만 웹 `12 KST`·DOCX `11 KST`로 다릅니다. 어느 자료가 틀렸는지, 또는 DOCX가 오래됐거나
> 아직 동기화되지 않았는지는 **단정하지 않으며**, 정확한 불일치 원인은 확인되지 않았습니다.
> runtime은 발표일자·발표시각 분기 없이 slot presence만 처리하므로 이 표기 차이의 영향을 받지
> 않고, 실제 JSON의 POP 반환 시점·형태는 후속 live 통합 검증 대상입니다.
...
- category·단위·범주·Missing 규칙은 공식 활용가이드 원문으로, 초단기예보 POP 포함 여부는 두 공식
  자료(API 허브 웹 변수 목록·활용가이드 `_260623.docx`)에서 확인했으나(제공 시작 시각 표기는 웹
  `12 KST`·DOCX `11 KST`로 다름 — 위 "공식 자료별 POP 표기" 참조), 인증된 실제 JSON 응답으로의
  end-to-end 검증(방어적 허용 및 초단기 POP 반환 시점·형태 포함)은 실제 키가 필요하며 후속 live
  통합 검증 과제입니다.
...
- 초단기예보 POP 정책 정정: 기상청 API 허브 `2.2 초단기예보` 변수 목록이 POP를 2026-06-23 12 KST
  이후 제공한다고 명시(확인일 2026-07-17). "초단기예보 POP 미제공" 서술은 잘못된 설명이었으므로 제거.
...
v1.2 / PR #6 branch / 2026-07 (공식 자료 POP 시작 시각 불일치 정정)
- 현재 공식 자료가 **모두** 초단기예보 POP를 포함함을 명시: API 허브 웹 변수 목록은 `12 KST`,
  API 허브 활용가이드 DOCX(`_260623.docx`, SHA-256 재검증)의 초단기예보 코드값 표에도 POP가 있으며
  주석은 `11 KST`. 이전 문서가 DOCX 쪽 POP를 부재/구버전으로 설명하던 서술을 정정.
- 차이는 POP 포함 여부가 아니라 제공 시작 시각(웹 12 KST vs DOCX 11 KST)이며, 어느 자료가 틀렸는지·
  정확한 불일치 원인은 추정하지 않고 미확인으로 기록. "공식 자료별 POP 표기" 표 추가.
- 서로 다른 배포본(공공데이터포털 2607 ZIP / 그 내부 DOCX / API 허브 `_260623.docx`)의 SHA-256을
  혼합하지 않고 각각 별개 자료로 기록.
- runtime 무변경: 발표일자·발표시각 분기 없이 slot presence만 처리하므로 표기 차이의 영향 없음
  (코드·테스트 assertion·의존성 무변경, 문서 한정).
```
