...
- **no request plan / no second request.** primary/previous request 객체를 만들지 않고 두 번째 HTTP
  ...
  **PR #19에서 이뤄졌습니다** — `createKmaHourlyFallbackService`
  ...
  단계 fallback). 그 orchestration이 생겼어도 이 classifier 자체는 여전히 pure하며(network·request 실행
  없음), production composition에는 아직 연결되지 않았습니다.
...
3. production composition wiring (PR #20).
4. `WeatherOverview` / `SourceMetadata` assembler와 final result selection.
