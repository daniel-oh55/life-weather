...
- **no request plan / no second request.** primary/previous request 객체를 만들지 않고 두 번째 HTTP
  요청을 실행하지 않습니다. PR #18에서 **request plan은 별도 building block**
  (`createKmaFallbackRequestPlanFactory`, [kma-fallback-request-plan.md](./kma-fallback-request-plan.md))으로
  추가됐지만, 이 classifier는 request plan을 **입력받지 않습니다** — classifier의 입력은 여전히 primary
  service result(`KmaHourlyForecastServiceResult`) 하나뿐입니다. request plan의 `previous` request는
  이 classifier가 **eligible**로 판정한 경우에만 orchestration에서 사용되며, 두 building block의 조합은
  **PR #19에서 이뤄졌습니다** — `createKmaHourlyFallbackService`
  ([kma-hourly-fallback.md](./kma-hourly-fallback.md))가 이 classifier를 **primary service result에 정확히
  1회** 적용하고, eligible일 때만 previous를 실행하며, **previous 결과는 다시 classify하지 않습니다**(한
  단계 fallback). **PR #20 grid fallback composition**
  ([kma-hourly-fallback-composition.md](./kma-hourly-fallback-composition.md))이 이 classifier를 PR #19
  service에 **명시적으로 주입**(fixed production eligibility policy)하지만, exact `03`/empty-hourly 정책과
  classifier 자체의 순수성(network·request 실행 없음, fresh 결과)은 **변경되지 않습니다.** `/weather`
  route와 location(위·경도) fallback에는 아직 연결되지 않았습니다.
...
- **classifier 자체는 production 동작을 바꾸지 않음.** classifier는 순수 분류 component이며
  composition·실행을 소유하지 않습니다. 기존 grid/location scheduled root는 계속 호출당 KMA request 최대
  1회로 불변이고, **PR #20 grid fallback root**만 eligible primary에 한해 previous를 한 번 더 호출해 최대
  2회가 될 수 있습니다(기존 두 root와 신규 fallback root 모두 아직 startup/route 미연결).

## 다음 orchestration PR 권장 범위

1. ~~PR #16 candidates를 조합하는 request plan~~ — **PR #18에서 완료**
   (`createKmaFallbackRequestPlanFactory`, [kma-fallback-request-plan.md](./kma-fallback-request-plan.md)).
2. ~~PR #18 request plan + PR #17 eligibility로 primary 한 번 + previous 최대 한 번의 fallback
   orchestration(AbortSignal pass-through 포함)~~ — **PR #19에서 완료**
   (`createKmaHourlyFallbackService`, [kma-hourly-fallback.md](./kma-hourly-fallback.md)).
3. ~~production composition wiring~~ — **PR #20에서 완료** (grid fallback composition은 이 classifier를
   PR #19 service에 명시적으로 주입, [kma-hourly-fallback-composition.md](./kma-hourly-fallback-composition.md)).
4. location(위·경도) fallback facade/composition.
5. `WeatherOverview` / `SourceMetadata` assembler와 final result selection.
6. `/weather` route와 HTTP mapping.
7. cache / stale-data.
...
v2 / PR #20 / 2026-07
- PR #20 grid fallback composition이 이 classifier를 PR #19 service에 명시적으로 주입
- exact 03/empty-hourly 정책과 classifier 순수성 불변
- /weather route·location fallback에는 미연결
```
