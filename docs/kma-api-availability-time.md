...
이 문서는 `@life-weather/weather-core`가 기상청(KMA) 단기예보·초단기예보의 **공식 API 제공 지연**을
반영해, 호출자가 제공한 절대 시각을 기준으로 **API 제공 지연 기준(availability-delay threshold)을 이미
통과한 가장 최근 발표시각**(`base_date` / `base_time`)을 선택하는 순수 함수
...
- 이 selector는 거기에 활용가이드의 별도 `API 제공 시간`(자료 제공 지연)을 얹어, 다음 조건을 충족하는
  가장 최근 issuance를 선택합니다.

```text
공식 발표시각 + product별 공식 API 제공 지연  ≤  reference instant
```

즉 "발표는 됐고, 문서화된 제공 지연도 지난" 가장 최근 발표시각을 고르는 **schedule 기반 availability
후보**입니다.
...
```text
호출자가 제공한 절대 instant
  → product별 공식 API 제공 지연만큼 reference를 과거로 이동 (reference − delay)
...
2. product별 API 제공 지연 밀리초를 선택합니다(SHORT 10분, ULTRA 15분).
3. adjusted reference `referenceEpochMilliseconds − delay`를 계산합니다.
...
활용가이드 `# 예보 발표시각` 절은 각 product의 발표 일정과 함께 `API 제공 시간 (~ 이후)`을 명시합니다.
이 selector는 그 값을 **고정 지연**으로 반영합니다.

| product | 발표시각(KST) | 공식 API 제공 시각 | 고정 지연 |
| --- | --- | --- | --- |
| `SHORT_FORECAST` (`getVilageFcst`) | `0200/0500/…/2300` (1일 8회) | 각 발표시각 `~10분` 이후 (예 `~02:10`) | **10분** |
| `ULTRA_SHORT_FORECAST` (`getUltraSrtFcst`) | 매시간 `HH30` | 매시간 `~HH45` 이후 | **15분** |
...
object/Map/cache/module singleton을 만들지 않습니다. **safety margin은 추가하지 않습니다** — 정확히
공식 문서상 지연(10/15분)만 반영합니다.

## threshold는 inclusive

`발표시각 + delay`와 정확히 같은 순간에 그 issuance가 선택 가능해집니다. 입력의 초·밀리초까지 비교에
반영합니다.

| product | reference (KST) | 결과 |
| --- | --- | --- |
| SHORT | `05:09:59.999` | 당일 `0200` (0500은 아직 제공 전) |
| SHORT | `05:10:00.000` | 당일 `0500` (제공 지연 통과) |
| SHORT | `05:10:00.001` | 당일 `0500` |
| ULTRA | `06:44:59.999` | 당일 `0530` (0630은 아직 제공 전) |
| ULTRA | `06:45:00.000` | 당일 `0630` (제공 지연 통과) |
...
| --- | --- | --- |
| 의미 | reference와 같거나 이전인 가장 최근 **발표 예정** issuance | 문서화된 API 제공 지연을 이미 통과한 가장 최근 issuance |
| 계산 | reference에 직접 발표 일정 적용 | `reference − delay`에 발표 일정 적용 |
...
schedule selector는 이 PR에서 availability delay를 **얻지 않습니다.** availability delay는 오직 이 신규
selector에서만 적용됩니다.
...
- **availability adjustment 이후** 선택된 `base_date` 연도가 `[1000, 9999]` 밖(예 `1000-01-01` 하한에서
  `reference − delay`가 첫 발표시각 이전으로 밀려 `0999`로 rollover)
...
- 문서화된 API 제공 지연 threshold를 아직 통과하지 않은 최신 issuance를 선택하지 않음
...
이 selector는 **schedule 기반 availability 후보**이자 **deterministic availability-delay policy**일
뿐입니다. **no safety margin**, **no live availability guarantee**, **no retry/fallback**입니다.
...
10분/15분 지연의 직접 근거는 활용가이드 `# 예보 발표시각` 절의 `API 제공 시간 (~ 이후)`으로, 단기예보
`~02:10, ~05:10, …`(발표 +10분), 초단기예보 `~HH45 이후`(발표 +15분)입니다. 이 값은 이미
[kma-issue-time.md](./kma-issue-time.md)에 기록된 발표 일정 근거와 동일한 hash 검증 가이드에서
확인된 것입니다.
...
- 이 selector는 문서화된 제공 지연만 반영할 뿐, 실제 API 가용성을 probe하거나 보장하지 않습니다.

## 아직 연결되지 않음 / 후속 wiring

이 PR은 순수 selector만 추가합니다. 다음은 이 PR에서 **연결하지 않았습니다.**

- **request factory에 미연결.** `apps/api`의 `createKmaForecastRequestFactory`는 계속 PR #8
  `selectLatestKmaForecastBaseTime`를 사용하며, method 이름 `createScheduledRequest`도 유지됩니다. 이
  신규 selector는 아직 주입/소비되지 않습니다([kma-forecast-request-factory.md](./kma-forecast-request-factory.md)).
- **production composition에 미연결.** scheduled/location composition은 이 selector를 아직 소비하지
  않습니다. runtime 동작은 PR #13과 동일합니다.
- **route 미구현.** `/weather` route·HTTP status mapping·`WeatherOverview`/`SourceMetadata` 조립은
  여전히 후속입니다.

후속 PR 권장 범위:

1. availability-delay selector를 소비하는 명시적 request factory 또는 명시적 wiring
2. empty-data 또는 publication-in-progress fallback 정책
...
```text
v1 / PR #14 / 2026-07
- SHORT 공식 API 제공 지연 10분 반영
- ULTRA 공식 API 제공 지연 15분 반영
