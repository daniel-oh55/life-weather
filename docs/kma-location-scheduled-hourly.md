...
factory, issue-time selector, hourly service, hourly normalizer, scheduled facade, location facade,
location composition. reference clock `2026-07-18T05:00:00.000+09:00`, 서울 좌표
`{ latitude: 37.5665, longitude: 126.978 }`가 PR #12 기대 grid `{ nx: 60, ny: 127 }`로 투영되고,
selector가 `base_date=20260718`/`base_time=0500`을 선택해 `getVilageFcst`를 호출한 뒤 하나의 정규화된
`HourlyForecast`(`2026-07-18T06:00:00+09:00`, `CLEAR`, 25.5 ℃ …)를 만드는 것을 검증합니다.
...
- **country/provider 선택 / location ID / 지역명 / 행정구역 검색 / reverse geocoding / GPS / location
  storage** — 없음.
- **기존 facade/composition 교체** — 없음(추가만 함).
- **grid converter 수식·request factory·Provider·normalizer·issue-time 변경** — 없음.
- **availability delay / retry / fallback / cache / stale-data** — 없음.
...
3. API availability delay / fallback / retry.
