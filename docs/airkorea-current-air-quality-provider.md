...
이 rollover는 canonical 결과가 `dataTime`의 4자리 `YYYY` 형식이 표현 가능한 최대 연도(`9999`) 이내일
때만 성립합니다 — `9999-12-31 24:00`은 5자리 연도(`10000`)가 필요해 표현 불가능하므로 다른 값과
동일하게 malformed로 거부됩니다(`9999-12-31 23:59`는 그대로 유효).
