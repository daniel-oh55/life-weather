...
이 계약(`contracts`)에는 **정규화된 수치만** 존재합니다(`precipitationAmountMillimeters`는
mm, `snowfallAmountCentimeters`는 cm, 확인된 "없음"은 `0`, 미제공·파싱 실패는 `null`). KMA
원본 범주 문자열은 계약에 넣지 않으며, `weather-core` 내부 매핑에만 존재합니다.

이 규범을 실제로 구현하는 **순수 파서**는 PR #3에서 `@life-weather/weather-core`에 추가되었습니다
(`parseKmaPrecipitationAmountMillimeters` → mm, `parseKmaSnowfallAmountCentimeters` → cm).
아래 규칙을 동일하게 따릅니다.
...
| `T 미만` (예: PCP `1mm 미만`, SNO `0.5cm 미만`) | `T / 2` |
| `L~U` (범위, **PCP만**) | 하한 `L` |
| `T 이상` | 하한 `T` |
| Missing 센티넬(`+900 이상`/`-900 이하`, 즉 수치 `>= 900`) | `null` |
| 미제공·통신장애·파싱 불가 | `null` |

- **no-amount 표기.** 공식 가이드는 `-`, `0`, `0.0`, `강수없음`/`적설없음`을 "없음"으로 범주화하므로
  이들은 확인된 `0`으로 정규화합니다.
- **Missing 센티넬.** 공식 `+900 이상`/`-900 이하`는 자료 없음이므로, 파싱된 수치 성분이 `>= 900`
  이면 `null`입니다.
- **범위.** Provider의 공식 원본 형식이 범위를 제공하면 `L~U` → 하한 `L`로 정규화합니다. **KMA
  PCP는 범위를 제공**하지만, **KMA SNO는 2607 가이드에서 범위를 제공하지 않으므로 범위 문자열을
  거부**(`null`)합니다.
- **JavaScript `null`의 현재 parser 의미.** 위 파서의 인자 JavaScript `null`/`undefined`는 "호출자
  측 데이터 미제공"으로 보아 `null`을 반환하며, 공식 no-amount(`-`/`0`)의 `0`과 구분합니다. 파서
  단독으로는 "필드 존재 + 값 null"과 "필드 미존재"를 구분할 수 없으므로 `null`을 `0`으로 바꾸지
  않습니다.
- **후속 과제(raw Provider의 field presence 보존).** 실제 HTTP Provider(원본 값을 위 파서에 넘겨
  계약 수치를 채우는 계층)는 아직 미구현이며 PR #4 범위입니다. Provider는 원본 응답의 **필드 존재
  여부(field presence)**를 보존한 상태에서 (1) 필드 존재 + 공식 null, (2) 필드 미존재, (3) 문자열
  `-`, (4) 숫자/문자열 `0`을 각각 구분해 처리해야 합니다.
- Provider 원본 범주 문자열은 공통 계약에 추가하지 않습니다. 코드 표·출처·경계 규칙은
