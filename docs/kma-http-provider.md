...
않습니다.** 따라서 reader를 성공적으로 획득한 뒤에는 모든 종료 경로에서 **명시적으로
`reader.releaseLock()`을 호출**해 body를 unlocked 상태로 되돌립니다. 대상 경로:

```text
정상 완독            → releaseLock (body.locked === false)
정확히 max bytes     → releaseLock (body.locked === false)
overflow (cancel 후) → releaseLock (body.locked === false)
첫 read 오류         → cancel 후 releaseLock
중간 read 오류       → cancel 후 releaseLock
cancel reject/throw  → 기존 결과 유지 + 가능하면 releaseLock
```

- `releaseLock()`은 읽기 루프를 감싼 `try/catch/finally`의 `finally`에서 호출하므로, overflow의
  early return이나 정상 완료 return에서도 항상 실행됩니다.
- `releaseLock()` 실패는 raw transport detail이므로 **삼켜서** 공개하지 않고, **이미 확정된 결과
  (`RESPONSE_TOO_LARGE`/`BODY_READ_ERROR`/성공)를 덮어쓰지 않습니다.** cancel 실패도 마찬가지로 기존
  결과를 덮어쓰거나 노출하지 않습니다.
