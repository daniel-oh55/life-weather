...
> 참고: 위 공식 coverage 안내 값은 실제로 **극단 격자 cell들의 중심 좌표**입니다. `31.651814`는
> 격자 `(149,1)`의 중심 위도, `43.393490`·`123.310165`는 격자 `(1,253)`의 중심 위도·경도,
> `132.774963`은 격자 `(149,253)`의 중심 경도입니다(공식 격자→위경도 역변환으로 교차 확인).
...
공식 격자 위치자료·공식 변환 결과에 대조한 값입니다.

| 위치 | 위도 | 경도 | 격자 (nx, ny) | 근거 |
| --- | --- | --- | --- | --- |
| origin 불변식 | `38` | `126` | `43, 136` | `OLAT`/`OLON` → `XO`/`YO` (투영 정의) |
| 서울 | `37.5665` | `126.978` | `60, 127` | 공식 DFS 대표 좌표 |
| 부산 | `35.1796` | `129.0756` | `98, 76` | 공식 DFS 대표 좌표 |
| 제주 | `33.4996` | `126.5312` | `53, 38` | 공식 DFS 대표 좌표 |
| API 허브 예시 | `36.5` | `127.5` | `69, 104` | API 허브 문서 예시 입력 |
| 인천 | `37.4563` | `126.7052` | `55, 124` | 공식 DFS 대표 좌표 |
| 대전 | `36.3504` | `127.3845` | `67, 100` | 공식 DFS 대표 좌표 |
| 광주 | `35.1595` | `126.8526` | `58, 74` | 공식 DFS 대표 좌표 |

### 네 격자 경계 fixture

극단 격자 cell들은 대부분 관할 행정구역이 없는 해상·국경 cell이라 행정구역 기반 격자 위치자료에
직접 나타나지 않습니다. 따라서 각 모서리의 위·경도는 **공식 격자 → 위·경도 변환**(KMA 허브가
수행하는 역방향 DFS)으로 얻은 해당 cell의 중심 좌표를 사용합니다. 그 극단 성분이 위 공식 coverage
안내 값(`31.651814`·`43.393490`·`123.310165`·`132.774963`)과 6자리까지 일치하므로, 이 좌표들은
정방향 수식에서 만들어낸 값이 아니라 **공식 좌표**임을 독립적으로 교차 확인할 수 있습니다.
...
못했습니다. 다만 구현에 사용한 DFS 상수·격자 범위(`nx 1~149`, `ny 253`, 5km, LCC `30/60/126/38`,
